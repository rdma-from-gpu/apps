...
void on_exit(int signal) { stop = true; }
...
inline void rate_limiter(uint64_t sent_pkts, bool print,
                         uint64_t interval = 10'000,
                         uint64_t print_interval = 1'000'000'000) {
    uint64_t t = now();
    if (start_to_send == 0) {
        start_to_send = t;
    ...
    double rate =
        write_size * 8.0 * (sent_pkts - last_count) / (1.0 * t - last_check);
    ...
    while (rate > max_rate) {
        ...
        //     usleep_time*sleeps);
        rate = write_size * 8 * (sent_pkts - last_count) /
               (1.0 * now() - last_check);
    }
...
int main(int argc, char **argv) {
    ...
    bool use_gpu = (mode == "gpu" || mode_cqe == "gpu" ||
                    data_location == "cuda" || buffer_location != "host");
    ...
    if (data_location == "cudaMallocHostPW")
        CUDA_CALL(
            cudaMallocHost(&driver_data, driver_data_size,
    ...
    if (driver_data == nullptr) {
        if (data_location == "cudaMallocHost") {
            LOG(INFO)
                << "driver data is being allocated through cudaMallocHost";
            CUDA_CALL(cudaMallocHost(&driver_data, driver_data_size,
                                     cudaHostAllocPortable |
                                         cudaHostAllocMapped |
                                         cudaHostAllocWriteCombined));
    ...
    // Init rdma
    // This is not portable, but allows to pick one local IP when we don't
    // specify it in the command line THese are local IPs in our testbed, most
    // likely they won't make any sense to you But if any of these is set in
    // your machine in a port that **you don't want to use**, please remove it
    // from here!
    std::vector<std::string> some_ips({"192.168.128.1", "192.168.129.1",
    ...
    ibv_qp *qp;
    ibv_pd *pd = nullptr;
    CHECK(init_rdma(&qp, &pd, some_ips, remote_address) == 0)
    ...
    if (gpu_batch == 1) {
        ...
        for (uint64_t i = 0; i < n && !stop; i += gpu_batch) {
            ...
            set_start_time<<<1, 1>>>();
            rdma_write_with_imm_kernel_multiple<<<1, 1, 1>>>(
                data,
                (void *)mr->addr,   // buffer
                write_size,         // size
                mr->lkey,           // lk
    ...
    if (mode == "gpu") {
        ...
        CUDA_CALL(cudaMemcpyFromSymbol(&send_time_sum, gpu_exec_time_sum,
                                       sizeof(send_time_sum), 0,
                                       cudaMemcpyDeviceToHost));
        CUDA_CALL(cudaMemcpyFromSymbol(&send_count, gpu_exec_count,
                                       sizeof(send_count), 0,
                                       cudaMemcpyDeviceToHost));
        // LOG(INFO) << "SUM " << send_time_sum;
        // LOG(INFO) << "COUNT " << send_count;
        // LOG(INFO) << "SO  " <<send_time_sum << "/ (1e-6 * " << send_count<< "
        // * " << prop.clockRate;

        double send_time_gpu_avg =
            send_time_sum / (1e-6 * send_count * prop.clockRate);
    ...
    if (mode_cqe == "gpu") {
        ...
        CUDA_CALL(cudaMemcpyFromSymbol(&cqe_time_sum, gpu_cqe_time_sum,
                                       sizeof(cqe_time_sum), 0,
                                       cudaMemcpyDeviceToHost));
        CUDA_CALL(cudaMemcpyFromSymbol(&cqe_count, gpu_cqe_count,
                                       sizeof(cqe_count), 0,
                                       cudaMemcpyDeviceToHost));
        CUDA_CALL(cudaDeviceSynchronize());

        double cqe_time_gpu_avg =
            cqe_time_sum / (1e-6 * cqe_count * prop.clockRate);
